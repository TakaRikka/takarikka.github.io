...
function startup() {
   ...
   for (i=0; i<ary_TitleData.length; i++) {
      ary_SortedData[i] = {};
      ary_SortedData[i]['id'] = i;
      ary_SortedData[i]['title'] = ary_TitleData[i];
   }
   ary_SortedData.sort(function(a,b) {
      var upA = a.title.toUpperCase();
      var upB = b.title.toUpperCase();
      return (upA < upB) ? -1 : (upA > upB) ? 1 : 0;
   });
   // タイトルから選択用チェックボックスに変換
   for (i=0; i<ary_SortedData.length; i++) {
      // Row[i]
      if ((i % int_Colspan) == 0) {
         var new_row = tbl_body_Select.insertRow(tbl_body_Select.rows.length);
         new_row.id = 'optSelRow' + ary_SortedData[i].id;
      }
      // Col[0]
      ...
      new_CheckBox.setAttribute('checked', 'false', 0);
      new_CheckBox.value = ary_SortedData[i].title;
      new_CheckBox.title = ary_SortedData[i].title;
      new_CheckBox.id = 'optSelect' + ary_SortedData[i].id;
      new_cell.appendChild(new_CheckBox);

      var new_span = cE('span');
      new_span.appendChild(cT(ary_SortedData[i].title));
      new_span.title = ary_SortedData[i].title;
      new_span.id = ary_SortedData[i].id;
      sC(new_span, 'cbox');
   ...
   new_CheckBox.onclick = function() {if (sanityCheck()) {chgAll();}}
   new_cell.appendChild(new_CheckBox);

   var new_span = cE('span');
   new_span.className = 'cbox bold';
   new_span.id = '_all';
   new_span.onclick = function() {chgFlag(new_span.id);if (sanityCheck()) {chgAll();}}
   ...
   for (i=0; i<ary_TitleData.length; i++) {
       gID('optSelect' + i).onclick = function() {notAll();}
   }
...
function sanityCheck() {
   var crazy = true;
   if (gID('optSelect_all').checked == true) {
      if (confirm('This will result in *thousands* of matches. Please confirm you are a masochist.')) {
         crazy = true;
      } else {
         crazy = !crazy;
...
function init(){
   ...
   for (i=0; i<ary_CharacterData.length; i++) {
      for (j=0; j<ary_TitleData.length; j++) {
         if (gID('optSelect' + j).checked && (ary_CharacterData[i][3][j] == 1)) {
   ...
   if (gID('optAgony').checked) {
      gID('divMiddleT').onclick = '';
      gID('divMiddleT').className = 'disabled';
      gID('divMiddleB').onclick = '';
      gID('divMiddleB').className = 'disabled';
   }
   gID('divMiddleT').innerHTML = str_CenterT;
   gID('divMiddleB').innerHTML = str_CenterB;
...
function imginit() {
   ...
   for (i=0; i<ary_CharacterData.length; i++) {
      ...
      if (ary_CharacterData[i][4].length > 0) {
         //for (j=4; j<ary_CharacterData[i].length;j++) {
            var new_img = cE('img');
            new_img.src = str_ImgPath + ary_CharacterData[i][4];
	 ...
	 csort2[i] = int_Result; // v2a
         ...
         if ((int_ResultImg != 0) && (i < int_ResultRank)) {
            ...
            if (obj_TempData[3].length > 0) {
               new_img.src = str_ImgPath + obj_TempData[Math.floor(Math.random() * (obj_TempData.length -4)) + 4];
            ...
            var div_cv = cE('span');
            div_cv.className = 'cv';
            div_cv.appendChild(cT(ary_TempData[ary_SortData[0][i]][2]));
            div_name.appendChild(div_cv);
         } else {
      ...
      for (i=0; i<2; i++) {
         ...
         if ((obj_TempData[4].length > 0) && gID('optImage').checked) {
            var obj_Item = cE("img");
            obj_Item.src = str_ImgPath + obj_TempData[Math.floor(Math.random() * (obj_TempData.length - 4)) + 4];
...
function OpenWnd() {
      ...
      with (oWindow.document) {
            write("<html>");
            write("<head>");
            write("<title>Granblue Sort: Raw Text Results<\/title>");
            write("<\/head>");
